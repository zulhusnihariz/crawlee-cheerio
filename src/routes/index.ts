...
router.addDefaultHandler(async ({ $, log, request }) => {
  ...
  switch (url.host) {
    case HOSTNAME.AMBCRYPTO:
      ...
      data.htmlContent = $('div.single-post-main-middle').children('p:not(:has(img))').toString()
      break
    case HOSTNAME.BEINCRYPTO: {
      data.content = $('div.entry-content-inner').children('p').text().trim()
      data.htmlContent = $('div.entry-content-inner').children('p:not(:has(img))').toString()
      break
    }
    case HOSTNAME.BIT_COLUMNIST: {
      ...
      data.htmlContent = $('div.post-content').children('p:not(:has(img))').toString()
      break
    }
    ...
    case HOSTNAME.CRYPTO_POTATO: {
      ...
      data.htmlContent = $('div.coincodex-content').children('p:not(:has(img))').toString()
    ...
    case HOSTNAME.CRYPTO_SLATE: {
      ...
      data.htmlContent = $('article.full-article').children('p:not(:has(img))').toString()
      break
    }
